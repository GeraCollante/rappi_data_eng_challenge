...
def create_currency_df(filepath: str) -> pd.DataFrame:
    """
...
if __name__ == "__main__":
    ...
    currency_df = create_currency_df(CURRENCY_PATH)

    df["value"] = df.apply(
        lambda x: usd_currency(x["total"], x["created"].strftime("%Y-%m-%d")), axis=1
    )
    ...
    output_path = save_csv(producto_df, "producto.csv")
    ...
    output_path = save_csv(ts_df, "time_series.csv")
